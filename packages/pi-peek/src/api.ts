...
/** @internal Initialize the session-scoped service when the extension starts. */
export function initPeekAPI(deps: PeekDeps): PeekAPI {
  ...
  let closed = false;
  const capture = (includeThinking = false) => {
    if (closed) throw new Error("peek: session has closed.");
    return new SessionSnapshot(deps.sessionManager.buildSessionProjection().messages, {
      includeThinking,
    });
  };
  const api: LiveAPI = {
    createInvestigation(options = {}) {
      ...
      const inner = createInvestigation({
        snapshot: capture(options.includeThinking),
        model,
        config: cfg,
        stream: (context, options) =>
          roles.streamWithRole(cfg.role, context, { ...options, model }),
      });
    ...
    serializeMainConversation(options = {}) {
      const snapshot = capture(options.includeThinking);
      try {
...
export function getPeekAPI(): PeekAPI {
  ...
  if (!api)
    throw new Error("PeekAPI not initialized. Load @d3ara1n/pi-peek and wait for session_start.");
  return api;
}

/** @internal Probe shared state without requiring an initialized session. */
