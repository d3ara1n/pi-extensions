/**
 * Spawn a pi child process and collect structured output.
 *
 * Uses pi's --mode json to get a JSON event stream.
 * Adapted from the official subagent example.
 */
 ...
 *
 * @param modelRef - Model identifier like "anthropic/claude-sonnet-4"
 * @param task - The task prompt
 * @param options - Spawn options
 * @returns SubagentResult with output and metadata
 */
				...
				if (msg.role === "assistant") {
					...
					for (const part of msg.content) {
						if (part.type === "text" && part.text) {
							result.output = part.text; // Keep last assistant text
						}
					}
				}
			}
