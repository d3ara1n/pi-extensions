...
export function registerPeekTools(pi: ExtensionAPI): void {
	...
	pi.registerTool({
		...
		async execute() {
			...
			return textResult(`${peers.length} peer${peers.length === 1 ? "" : "s"} online\n` + fullPeerList(peers));
		},
	});
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			...
			at: Type.Optional(
				Type.String({
					description: "Target instance name (e.g. 'Fox'). Omit to auto-pick the other same-project instance.",
				}),
			),
			sessionId: Type.Optional(
				Type.String({ description: "Pin a specific instance by sessionId (use when names collide)." }),
			),
		}),
		...
		renderResult(result, { expanded }, theme, context) {
			...
			const body = isError ? theme.fg("error", firstLine.slice(0, 100)) : theme.fg("dim", firstLine.slice(0, 100));
		...
		async execute(_toolCallId, params, signal) {
			...
			if (Array.isArray(resolved)) {
				const cands = resolved
					.map((p) => `- ${p.name} · sessionId=${p.sessionId} · ${p.gitBranch ?? "(no branch)"} · ${p.cwd}`)
					.join("\n");
				return textResult(
