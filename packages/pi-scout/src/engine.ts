 ...
 * effects and may inject a message. Apply-time failures mark the decision as
 * an error and zero the offending field.
 *
 * @returns any injected message
 *
 * @internal — exported for testing.
 */
export async function applyDecision(decision: ScoutDecision, ctx: ScoutContext): Promise<AppliedDecision> {
  ...
  for (const m of MODULES) {
    ...
    const res = await m.apply(decision.fields[m.field] as never, ctx);
