...
|------|------|---------|------|
| success | `✓` | AI summary → output 首行（截断 70 字）→ `(no output)` 占位，按优先级取 | success + text（占位 muted） |
...
内容块（files / context / output / activity）为空则整块连同前导 Spacer(1) 一起跳过，避免双空行。结构性行（taskline / resultline / usage）不受此规则约束——条件满足时必显示，靠占位符兜底，永不因内容为空而消失。

| 内容 | 折叠 | 展开 |
|------|------|------|
| task 预览 | taskline 始终 | 同 |
| summary | resultline（成功时，按链取，永不为空） | 同 |
| files | — | 每行 `@path`（有才显示） |
| context | — | `ctx N chars`（有才显示） |
| task 全文 | — | dim |
| activity | 仅 running 显最近 5 条 | 总显示 |
| output | — | Markdown（完成时，永不为空：空则显示占位） |
...
用户配置单位为秒，默认 `timeout: 1500`（25 分钟）。`effectiveTimeout(role, baseTimeout)` 秒进秒出，唯一转 ms 处是调用 `spawnSubagent` 时 `* 1000`（`setTimeout` API 要求，不泄露到配置层）。`spawn.ts` 内部仍用 ms（内部 API，正确）。
